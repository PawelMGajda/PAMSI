...
using namespace std;
#define DataType int
                    // do wyboru int lub  double inne typy danych wymagaja wprowadzenia na koncu pliku sort_algorithms.cpp
...
{
    auto t_start =  chrono::high_resolution_clock::now(); //start
    fp(tab, tmp, 0, n-1, log(n) * 2);
    auto t_end =  chrono::high_resolution_clock::now(); //stop

    return  chrono::duration<double,  milli>(t_end - t_start).count(); //zwraca roznice czasu
}
